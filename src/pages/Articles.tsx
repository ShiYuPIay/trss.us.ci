  ...
  return (
    <main className="page-main">
      ...
      <section className="container article-browser">
        ...
        {filtered.length ? <Stagger className="article-grid">{filtered.map((article, index) => <ArticleCard key={article.slug} article={article} index={index} />)}</Stagger> : <FadeIn className="empty-state"><Search size={28} /><h2>暂时没有匹配的文章</h2><p>换个关键词，或清除标签筛选再试试。</p></FadeIn>}
      </section>
    </main>
  )
}
