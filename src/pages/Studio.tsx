...
import { CloudCog, FileUp, ImagePlus, Save } from 'lucide-react'
import { toast } from 'sonner'
import { FadeIn } from '@/components/MotionPrimitives'
import { MarkdownContent } from '@/components/blog/MarkdownContent'
import { createArticle } from '@/hooks/use-articles'
import { isCloudConfigured, uploadAsset } from '@/lib/cloudbase'
...
export default function Studio() {
  ...
  const upload = async (file: File, asCover = false) => {
    try {
      ...
      if (asCover) { setCover(asset.storageId); setCoverPreview(asset.previewUrl) }
      else setAttachments((items) => [...items, { name: file.name, url: asset.storageId, size: file.size, type: file.type }])
  ...
  return (
    <main className="page-main studio-page">
      ...
      {!isCloudConfigured && <div className="container"><div className="service-notice studio-notice"><CloudCog size={22} /><div><strong>云服务正在准备中</strong><span>你可以体验编辑与预览；登录、保存及文件上传会在云环境就绪后启用。</span></div></div></div>}
